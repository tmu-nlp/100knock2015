    ...
    for one_sent_list in all_sent_list:
        ...
        for chunk in one_sent_list:
            if chunk.dst != str(-1):
                ...
                for src in one_sent_list[int(chunk.dst)].srcs:
                    morph_sahen = one_sent_list[int(src)].morphs
                    if len(morph_sahen) == 2 and int(src) != count:
                        if morph_sahen[0].pos1 == "サ変接続" and morph_sahen[1].surface == "を":
                            sahen = morph_sahen[0].surface + morph_sahen[1].surface
                            break
                        else:
                            sahen = ""
                    else:
                ...
                if morph_kakarisaki_first.pos != "動詞":
                    ...
                    case_pattern_verb = sahen + morph_kakarisaki_first.base

                if case_pattern_verb and case_pattern_particle and sahen:
